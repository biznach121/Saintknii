      ...
      >
        ...
        <footer className="flex items-center gap-3 border-t border-[#422900]/12 px-6 py-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="h-12 flex-1 rounded-full bg-[#422900] text-sm font-semibold text-[#e9f4e5] transition-colors hover:bg-[#301e00]"
          >
            Show {resultCount}
          </button>
        </footer>
...
export function FilterGroup({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-7 last:mb-0">
      <p className="mb-3 text-[12px] font-bold uppercase tracking-wide text-[#638453]">{title}</p>
      {children}
    </div>
  );
}
